...
void fxFillDots (int c) {
    ...
    for (teta = 0; teta < max; teta++)
        ...
        p[lptr  ].x = (stardist * sintabf[((phi*rat) + (int)((lut_size >> 1) * sintabf[(c << 8) & (lut_size-1)])) & (lut_size-1)] + 32 * sintabf[((c << 10) + (teta << 12)) & (lut_size - 1)]);
        p[lptr  ].y = (stardist * costabf[((phi*rat) + (int)((lut_size >> 1) * sintabf[(c << 8) & (lut_size-1)])) & (lut_size-1)] + 32 * costabf[((c << 10) + (teta << 12)) & (lut_size - 1)]);
...
int main () {
    ...
    while (i < 256) {
        ...
        fx3dMovep(32 * sintabf[((i << 9)) & (lut_size - 1)], 32 * costabf[(i << 9) & (lut_size - 1)], 0);
        ...
        fx3dMove  (0,0, - (stardist << 2) - (stardist << 1));
