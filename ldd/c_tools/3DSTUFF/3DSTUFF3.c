...
void fxMakeSprite() {
    ...
    for (y = -(spr_size >> 1); y < (spr_size >> 1); y++) {
        for (x = -(spr_size >> 1); x < (spr_size >> 1); x++) {
            *((char*)sprptr++) = sat((int)(0x400 / ((x*x + y*y) + ee)), 192) & 0xFF;
        }
    }
}
...
void initpal() {
    ...
    for (i = 0; i < 256; i++) {
        pal[(i << 2)    ] = sat((i >> 3) + (i >> 2), 63);
        pal[(i << 2) + 1] = sat((i >> 2) + (i >> 5), 63); 
    ...
    for (j = 0; j < 4; j++) { 
        k += 16;
        for (i = 0; i < 240; i++) {
            r = sat(((16 * j) + pal[((((i * 256) / 240)) << 2)    ]), 63) >> 1;
            g = sat(((16 * j) + pal[((((i * 256) / 240)) << 2) + 1]), 63) >> 1;
            b = sat(((16 * j) + pal[((((i * 256) / 240)) << 2) + 2]), 63) >> 1;
...
int main () {
    ...
    while (i < 768) {
        ...
        fx3dMovep(64 * sintabf[((i << 9)) & (lut_size - 1)], 32 * costabf[(i << 9) & (lut_size - 1)], 0);
        ...
        fx3dMove  (0,0, - (stardist << 2) - (stardist << 0) + 32 * sintabf[((i << 9)) & (lut_size - 1)]);
