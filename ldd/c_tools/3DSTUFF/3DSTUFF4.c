...
void fxFillDots (int c) {
    ...
    for (teta = 0; teta < max; teta++) {
        //p[lptr  ].x = (stardist + 0 * sintabf[((c << 10) - (teta << 12)) & (lut_size - 1)]) * sintabf[((phi*rat) + (int)((lut_size >> 1) * sintabf[(c << 7) & (lut_size-1)])) & (lut_size-1)];
        //p[lptr  ].y = (stardist + 0 * sintabf[((c << 10) - (teta << 12)) & (lut_size - 1)]) * costabf[((phi*rat) + (int)((lut_size >> 1) * sintabf[(c << 7) & (lut_size-1)])) & (lut_size-1)];
    */    
    //    p[lptr  ].x = stardist * sintabf[((teta*rat) /*+ (int)((lut_size >> 1) * sintabf[(c << 8) & (lut_size-1)])*/) & (lut_size-1)];
    //    p[lptr  ].y = stardist * costabf[((teta*rat) /*+ (int)((lut_size >> 1) * sintabf[(c << 8) & (lut_size-1)])*/) & (lut_size-1)];
    //    p[lptr++].z = 64 * sintabf[((c << 10) + (teta << 13)) & (lut_size - 1)];
    //}
    /*
    for (teta = 0; teta < max; teta++)
...
void fxMakeSprite() {
    ...
    for (y = -(spr_size >> 1); y < (spr_size >> 1); y++) {
        for (x = -(spr_size >> 1); x < (spr_size >> 1); x++) {
            col = sat(192 * cos(sqrt(x*x + y*y)/30), 192);
            col = (col > 0) ? col : 0;
            *((char*)sprptr++) = col;
            //*((char*)sprptr++) = sat((int)(0x4000 / ((x*x + y*y) + ee)), 192) & 0xFF;
            //*((char*)sprptr++) = sat((256 - (x*x + y*y)), 192);
        }
    }
}
...
void sprdump() {
    ...
    ff = fopen("pal.bin", "wb");
    fwrite(&tspal, sizeof(unsigned char), sizeof(tspal), ff);
    fclose(ff);
}
...
void initpal() {
    ...
    for (i = 0; i < 256; i++) {
        //pal[(i << 2)    ] = sat((i >> 3) + (i >> 4), 63);
        //pal[(i << 2) + 1] = sat((i >> 3) + (i >> 3), 63); 
        //pal[(i << 2) + 2] = sat((i >> 3) + (i >> 4), 63);
        
        pal[(i << 2)    ] = sat((i >> 2), 63);
        pal[(i << 2) + 1] = sat((i >> 2), 63); 
        pal[(i << 2) + 2] = sat((i >> 2), 63);
    }
    
    k = 16;
    for (i = 0; i < 240; i++) {
        r = pal[((((i * 256) / 240)) << 2)    ] >> 1;
        g = pal[((((i * 256) / 240)) << 2) + 1] >> 1;
        b = pal[((((i * 256) / 240)) << 2) + 2] >> 1;
...
int main () {
    ...
    //while (i < 768) {
        ...
        fx3dMovep(32 * sintabf[((i << 9)) & (lut_size - 1)], 32 * costabf[((i << 8) + (i << 7)) & (lut_size - 1)], 0);
        ...
        //fx3dRotate((i << 8) & (lut_size - 1)/* + (int)((lut_size >> 2) * sintabf[(i << 7) & (lut_size - 1)]) & (lut_size - 1)*/,
        //           (i << 8) & (lut_size - 1)/* + (int)((lut_size >> 1) * sintabf[(i << 8) & (lut_size - 1)]) & (lut_size - 1)*/,
        //           (i << 9) & (lut_size - 1));
