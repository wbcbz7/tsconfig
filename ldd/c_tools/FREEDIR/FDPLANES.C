...
unsigned char  fdbuffer[64000], fdbuffer2[64000];
unsigned char  fdlightmap[64000];

unsigned char  *buffer1 = &fdbuffer, *buffer2 = &fdbuffer2, *buffer3;
...
void fdBlur()  {
    long b1 = buffer1, b2 = buffer2;
    int  i, j, k;
    
    for (i = 0; i < 64000; i++) {
        *((char*)b1) = sat((*((char*)b1) + (*((char*)b2) >> 1)), 63);
    }
}
...
int main() {
    ...
    while (!kbhit()) {
        ...
        memcpy(screen, buffer1, 64000);
        
        buffer3 = buffer2; buffer2 = buffer1; buffer1 = buffer3;
        
        fdCalcPlanes((i << 2), (sintab[(i << 12) & 0xFFFF] >> 8), (sintab[((i << 3) + (i << 2)) & 0xFFFF]) >> 8, 
                    ((i << 7) & 0xFFFF), ((i << 7) & 0xFFFF), (((i << 7) + (i << 0)) & 0xFFFF));
